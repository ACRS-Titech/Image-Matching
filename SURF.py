...
#Input
sample_image = input("Input a sample image:")
template_image = input("Input the matching image:")

def ImageMaching(TargetImage, DataBaseImage):

    #EsttimatedCameraPosition
    #lat lon height
    position=[]
    
    # Load the images
    image = cv2.imread('{}'.format(sample_image))
    ...
    template = cv2.imread('{}'.format(template_image))
    ...
    # we don't have to visualize.
    # plt.imshow(img3),plt.show
    # plese return the camera position.
    return position

def main():
    CameraPosition = ImageMaching(sample_image,template_image)
